...
public sealed class TicketsSendingViewModel : ViewModelBase
{
        ...
        ProductsAndPricesService productsAndPricesService)
    {
        ...
        ResetPlatformWeightCommand = new RelayCommand(() => { TicketPlatformWeightText = "0"; SendingWeightText = string.Empty; });
    ...
    private async Task CreateSendingTicketHeaderAsync()
    {
        ...
        try
        {
            ...
            // If we created from New Buyer mode, switch back to normal ticket mode.
            if (IsNewBuyerOnly)
    ...
    private async Task FinalizeSendingTicketAsync()
    {
        ...
        await SearchSendingTicketsAsync();
        MoveSendingTicketToTop(ticketId);
        SelectedSendingTicket = SendingTicketSearchResults.FirstOrDefault(r => r.TicketId == ticketId) ?? SendingTicketSearchResults.FirstOrDefault();
